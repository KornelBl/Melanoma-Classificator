...
class LRFinder(Callback):
    ...
    def __init__(self, min_lr, max_lr, mom=0.9, stop_multiplier=None,
                 reload_weights=True, batches_lr_update=5, config:dict=None):
        self.min_lr = min_lr
        self.max_lr = max_lr
        self.mom = mom
    ...
    def on_train_end(self, logs=None):
        ...
        filename = os.path.join(get_log_dir(self.config), "learn_rates.npy")
        np.save(filename, np.array(self.learning_rates[:len(self.losses)]))
        filename = os.path.join(get_log_dir(self.config), "losses.npy")
        np.save(filename, np.array(self.losses))
